...
function ruleReference(rule, scope, home, project) {
  if (scope === 'global') {
    const relative = path.relative(home, rule.file);
...
export function planOpenCodeActions(scanResult, { copy = false } = {}) {
  ...
  if (scanResult.hookConfigs.length) {
    actions.push({ label: 'Claude hooks', status: 'skip', note: 'OpenCode plugins use different events, review manually' });
  }
...
export function renderOpenCodeReport(scanResult, actions, { apply = false } = {}) {
  const lines = ['', '📦 dsh-movein · Claude Code -> OpenCode safe move', ''];
  ...
  lines.push('');
  lines.push(`  found ${scanResult.skills.global.length + scanResult.skills.project.length} skills · ${scanResult.commands.global.length + scanResult.commands.project.length} commands · ${scanResult.agents.global.length + scanResult.agents.project.length} agents · ${scanResult.mcpServers.length} MCP servers · ${(scanResult.rules?.global.length ?? 0) + (scanResult.rules?.project.length ?? 0)} rules`);
