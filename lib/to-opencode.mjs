...
export function renderOpenCodeReport(scanResult, actions, { apply = false } = {}) {
  ...
  lines.push('');
  lines.push(`  found ${scanResult.skills.global.length + scanResult.skills.project.length} skills · ${scanResult.commands.global.length + scanResult.commands.project.length} commands · ${scanResult.agents.global.length + scanResult.agents.project.length} agents · ${scanResult.mcpServers.length} MCP servers · ${(scanResult.rules?.global.length ?? 0) + (scanResult.rules?.project.length ?? 0)} rules · ${scanResult.autoMemoryFile ? 1 : 0} memory`);
