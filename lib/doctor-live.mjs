...
export async function runLiveDoctor({
  ...
  try {
    ...
    child.stderr?.on('data', (chunk) => { output = (output + chunk.toString()).slice(-8_192); });
    ...
    if (outcome.type === 'ready') {
      ...
      const detail = outcome.last
        ...
        : '';
      checks.push({ level: 'bad', label: 'live DSH web baseline', note: `official baseline readiness timed out after ${timeoutMs}ms${detail}; ${observedCleanupPhrase(stop, observed)}${diagnostic(output)}` });
      ...
      checks.push({ level: 'bad', label: 'live DSH web baseline', note: `official baseline startup exited before readiness (code ${outcome.code ?? 'none'}${outcome.signal ? `, ${outcome.signal}` : ''})${diagnostic(output)}` });
    }
  } catch (error) {
