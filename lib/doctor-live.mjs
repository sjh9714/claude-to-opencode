...
function requestPage(url, timeoutMs, cookie = '') {
  const started = Date.now();
  return new Promise((resolve) => {
    const req = http.get(url, { headers: { accept: 'text/html', ...(cookie ? { cookie } : {}) } }, (response) => {
      // DSH rc.1 exchanges the launch token for a cookie at exactly `/`.
      // Follow that one same-origin hop only; never persist or print the cookie.
      const issuedCookie = (response.headers['set-cookie'] || []).map((value) => value.split(';', 1)[0]).join('; ');
      if (!cookie && response.statusCode === 303 && response.headers.location === '/' && issuedCookie && issuedCookie.length <= 8_192) {
        response.resume();
        resolve(requestPage(new URL('/', url), Math.max(1, timeoutMs - (Date.now() - started)), issuedCookie));
        return;
      }
      let body = '';
...
function requestJavascript(url, timeoutMs, cookie = '') {
  return new Promise((resolve) => {
    const req = http.get(url, { headers: { accept: 'text/javascript, application/javascript', ...(cookie ? { cookie } : {}) } }, (response) => {
      let bytes = 0;
...
export async function runLiveDoctor({
  ...
  try {
    ...
    if (outcome.type === 'ready') {
      ...
      const asset = await waitForHttp(page.assetUrl, {
        deadline, terminalPromise, timeoutPromise, aborted,
        request: (assetUrl, remaining) => javascriptRequest(assetUrl, remaining, page.cookie),
      });
