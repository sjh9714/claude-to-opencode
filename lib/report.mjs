...
export function renderReport(scanResult, actions, { apply }) {
  ...
  const counts = {
    ...
    commands: scanResult.commands.global.length + scanResult.commands.project.length,
  };
  ...
  if (!actions.length) {
    L.push(`  nothing to move. Is ${scanResult.origin || 'Claude Code'} set up on this machine (${scanResult.origin === 'Codex' ? '~/.codex' : '~/.claude'})?`);
    L.push('');
    return L.join('\n');
  }
  if (!apply) {
