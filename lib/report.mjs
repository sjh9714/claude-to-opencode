...
export function renderReport(scanResult, actions, { apply, starPrompt = false }) {
  const L = [];
  L.push('');
  ...
  for (const finding of hookSafetyFindings(scanResult.hookConfigs)) {
    L.push(`  ${finding.kind === 'canary' ? '○' : '⚠'} hook ${finding.message}`);
  }
  ...
  if (!apply) {
    ...
    if (!errors.length && actions.some((a) => a.status === 'done')) {
      ...
      if (starPrompt) L.push('  useful on your machine? Star it at https://github.com/sjh9714/dsh-movein');
    }
  }
  L.push('');
  return L.join('\n');
