...
function hookBridgeConfigPaths(patchText) {
  const paths = new Set();
  let row = [];
  const inspect = () => {
    if (!row.some((line) => /^\s*name:\s*'@deepseek-ai\/dsh-hooks-claude-code'\s*$/.test(line))) return;
    const configPath = row
      .map((line) => /^\s*configPath:\s*'((?:[^']|'')*)'\s*$/.exec(line))
      .find(Boolean);
    if (configPath) paths.add(configPath[1].replaceAll("''", "'"));
  };
...
export function runDoctor({ home = os.homedir(), project = process.cwd(), dshHome, profile = 'web' } = {}) {
  ...
  if (exists(manifestPath)) {
    ...
    warn('manifest', 'no movein-manifest.json; migration provenance is unavailable');
  }
  ...
  for (const file of [
    ...
    for (const groups of Object.values(settings.hooks ?? {})) {
      for (const g of Array.isArray(groups) ? groups : []) {
        const m = g?.matcher;
        if (typeof m === 'string' && /\b(?:Bash|Read|Write|Edit)\b/.test(m)) {
          warn('hook matcher', `"${m}" in ${file} contains Claude-style uppercase tool names; those alternatives cannot match DSH's lowercase tool names (#582)`);
        }
      }
    }
  }
  ...
  if (supportedHooks > 0) {
    const supportedConfigs = hookConfigs.filter((config) => supportedCommandHookCount([config]) > 0);
    const bridgePaths = new Set([...hookBridgeConfigPaths(patchText)].map(pathKey));
    const missingConfigs = supportedConfigs.filter((config) => !bridgePaths.has(pathKey(config.scope)));
    if (missingConfigs.length) {
      bad('hook bridge', `${supportedHooks} supported command hook(s) found, but no generated bridge row points to ${missingConfigs.map((config) => config.scope).join(', ')}`);
    } else {
      ok('hook bridge', `${supportedHooks} supported command hook(s) across ${supportedConfigs.length} settings file(s) have matching bridge rows; this proves wiring, not runtime enforcement`);
    }
