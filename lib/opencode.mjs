...
function mergeMcp(configs, notices) {
  ...
  for (const entry of merged.values()) {
    ...
    if (raw.type === 'local') {
      if (!Array.isArray(raw.command) || typeof raw.command[0] !== 'string' || !raw.command[0]) {
        notices.push({ kind: 'mcp', sourcePath, message: `malformed MCP ${name}, skipped` });
        continue;
      }
      out.push({
...
function instructionCandidate(configs, notices) {
  let source = null;
  for (const config of configs) {
    if (config.data.instructions !== undefined) source = { config, values: asArray(config.data.instructions) };
  }
  if (!source || !source.values.length) return null;

  const values = source.values.filter((value) => typeof value === 'string');
  const concrete = values.filter((value) => !isUrl(value) && !hasGlob(value));
  if (source.config.scope !== 'project' && values.length === 1 && concrete.length === 1) {
    const file = path.resolve(path.dirname(source.config.file), concrete[0]);
    if (exists(file)) return file;
  }
  notices.push({
    kind: 'instructions',
    sourcePath: source.config.file,
...
export function scanOpenCode({
  ...
  const projectRoot = path.join(project, '.opencode');
  ...
  return {
    ...
    errors,
    notices,
    globalClaudeMd: instructionCandidate(configs, notices),
    dshGlobalAgentsMd: exists(path.join(dshHome, 'AGENTS.md')),
    projectClaudeMd: exists(path.join(project, 'AGENTS.md')) ? path.join(project, 'AGENTS.md') : null,
    skills: splitSkills,
