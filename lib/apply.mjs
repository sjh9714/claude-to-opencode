...
function permsRow(permissions) {
  ...
  for (const k of ['deny', 'ask']) {
    if (!permissions[k].length) continue;
    lines.push(`        ${k}:`);
    for (const r of permissions[k]) lines.push(`          - ${yq(r)}`);
  }
  return lines.join('\n');
}
...
export function buildPatchBlock(scanResult, { withMcp = true, withHooks = true, withPerms = true } = {}) {
  ...
  if (withPerms && (scanResult.permissions.deny.length || scanResult.permissions.ask.length)) {
    rows.push(permsRow(scanResult.permissions));
  }
