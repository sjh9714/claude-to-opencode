...
const sameFileContents = (left, right) => {
  try {
...
function validPermissionToolName(tool) {
...
function parsePermissionRuleShape(raw) {
  ...
  if (!validPermissionToolName(tool)) return null;
...
export function emitRules(permissions) {
  const L = ['# generated by dsh-movein --emit-rules from Claude Code settings',
    ...
    'rules:'];
  const row = (r, action) => {
    const parsed = parsePermissionRuleShape(r);
    if (!parsed) return;
    const { tool, spec } = parsed;
    const glob = spec ? (spec.endsWith(':*') ? `${spec.slice(0, -2)}*` : spec) : null;
    if (tool === 'Bash') {
...
export function planActions(scanResult, { copy = false } = {}) {
  ...
  if (scanResult.globalClaudeMd) {
    ...
    if (scanResult.dshGlobalAgentsMd) {
      if (sameFileContents(target, scanResult.globalClaudeMd)) {
        a.push({
          label, status: 'skip', note: `${target} already matches the source; provenance will be recorded on apply`,
          kind: 'instructions', src: scanResult.globalClaudeMd, dest: target, recordExisting: true,
        });
      } else {
      ...
      a.push({
        ...
        kind: 'instructions', src: scanResult.globalClaudeMd, dest: target,
  ...
  if (buildPatchBlock(scanResult)) {
    ...
    a.push({
      ...
      kind: 'config', src: `${scanResult.origin || 'Claude Code'} configuration`, dest: patchPath,
...
// Where each moved asset/config came from - so provenance and destination
// health stay answerable later. Appended per apply run at the DSH home.
export function writeManifest(scanResult, actions) {
  const moved = actions.filter((a) => a.kind && (a.status === 'done' || (a.status === 'skip' && a.recordExisting)))
