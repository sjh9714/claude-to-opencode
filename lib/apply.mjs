...
export function agentToSkill(mdText, fallbackName) {
  const m = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/.exec(mdText);
  if (!m) return null;
  const get = (k) => {
  ...
  if (!name) return null;
  const description = get('description') || `Converted from the Claude Code subagent ${name}`;
  const skillMd = [
    '---',
    `name: ${name}`,
    `description: ${yq(description)}`,
    `whenToUse: ${yq(`Adopt the ${name} subagent persona from Claude Code. ${description}`)}`,
    '---',
    '',
    m[2].trim(),
    '',
...
export function commandToSkill(mdText, fallbackName) {
  ...
  const description = get('description')
    || body.trim().split('\n')[0].slice(0, 100)
    || `Converted from the Claude Code slash command /${name}`;
  ...
  const skillMd = [
    ...
    '---',
    '',
    `Converted from a Claude Code slash command. The user invokes this as /${name}${hint ? ` (arguments: ${hint})` : ''}; treat any text the user provides with it as $ARGUMENTS.`,
    '',
    body.trim(),
    '',
  ].join('\n');
...
export function planActions(scanResult, { copy = false } = {}) {
  ...
  for (const g of agentMoves) {
    ...
    let conv = null;
    try { conv = agentToSkill(fs.readFileSync(g.file, 'utf8'), base); } catch { /* unreadable */ }
    if (!conv) {
  ...
  for (const c of commandMoves) {
    ...
    let conv = null;
    try { conv = commandToSkill(fs.readFileSync(c.file, 'utf8'), base); } catch { /* unreadable */ }
    if (!conv) {
      a.push({ label: `command /${base}`, status: 'skip', note: 'unreadable, not converted' });
      continue;
    }
    const dest = path.join(c.destRoot, conv.name);
    if (exists(dest)) {
      a.push({ label: `command /${base}`, status: 'skip', note: `${dest} already exists` });
    } else {
      a.push({
        label: `command /${base}`, status: 'move', note: `convert to user-invocable skill -> ${dest}/SKILL.md`,
  ...
  if (buildPatchBlock(scanResult)) {
    ...
    a.push({
      ...
      exec: (installer = defaultInstaller) => {
        ...
        const dropped = [];
        for (const [key, pkgs] of [['withMcp', MCP_PKGS], ['withHooks', HOOKS_PKGS], ['withPerms', PERMS_PKGS]]) {
