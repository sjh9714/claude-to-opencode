...
// (measured on the rc.6 source). A hook on any other event never fires under
// DSH, and the bridge only runs type "command" hooks.
      ...
      for (const g of groups ?? []) {
        for (const h of g?.hooks ?? []) {
          if (h?.type && h.type !== 'command') {
            out.push({ scope: hc.scope, event, why: `type "${h.type}" is skipped by the bridge, only command hooks run` });
          }
        }
      }
    }
