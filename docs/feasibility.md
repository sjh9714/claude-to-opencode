...
| Hooks | Partial, verify before relying | First party `@deepseek-ai/dsh-hooks-claude-code` reads a configured Claude settings file, but maps only 7 of 30 events and command handlers. Unsupported events are silently skipped, DSH tool matchers are lowercase, Windows PowerShell can lose a native child's exit 2 (#2485/#3714), and `continue:false` is recorded without halting the run (#1514) |
...
6. A composed hook row only proves configuration wiring. `doctor` must remain static and non-invasive; actual deny enforcement needs a disposable exit-2 canary on the target OS and DSH version.

## Verification record

- 2026-08-15: real apply on this machine. 4 skills linked, AGENTS.md linked, cc-hooks row in `dsh --profile web --dump-config` under the home patch layer.
- 2026-08-15: `dsh web --port 3099` booted clean with the full generated patch, zero module errors, HTTP 200.
- 2026-08-27: migration fixtures, isolated live-doctor fixtures, and OpenCode `1.18.23` config/skill/agent loading passed.
- Not automated by design. A live model turn confirming hook denial would execute user-configured commands, so the compatibility guide gives a disposable manual canary instead. No hook enforcement claim is made from config composition alone.
